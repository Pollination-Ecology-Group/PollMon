"""
python scripts/annotate_images_Ireen_help.py

Simple tkinter GUI for drawing bounding boxes on insect images.
Used to generate YOLO-format .txt annotation files for training.
All config is done through the globals below, no CLI args needed.
"""
...
import torch
from ireen.subsidiary_connector import IreenProcessor, IreenDetectionModel, IreenConfig
...
MODEL_NAME = "ireen/ireen-detect-v8-high"  # "lite" variant also works, ~3x faster but less accurate
REQUIRE_CUDA = True        # will error out immediately if no GPU found
CUDA_DEVICE_INDEX = 0
USE_FP16 = False           # set True to save VRAM; can cause dtype issues on some setups
BOX_THRESHOLD = 0.35       # main confidence cutoff for detections
TEXT_THRESHOLD = 0.25      # text-to-region matching cutoff (lower = more permissive)
MIN_BOX_SIZE_PIXELS = 5    # ignore tiny boxes below this edge length
MAX_INFERENCE_SIZE = 800   # longest-side resize before feeding to the model
MAX_BOXES_PER_IMAGE = 5    # cap on how many boxes we keep after NMS
MAX_BOX_AREA_RATIO = 0.60  # skip boxes that cover more than 60% of the image
PREFETCH_AHEAD = 20        # how many upcoming images to predict in background
PREFETCH_WORKERS = 4       # worker threads for prefetch (GPU is the bottleneck anyway)

# Ireen-specific params (see docs for details, defaults are usually fine)
MAX_ALLOWED_THOUGHT_TIME = 2.5          # max seconds the reasoning loop runs per image
PREVIOUS_OBJECT_CROSS_REFERENCE = True  # compare detections w/ previous frame to stabilise boxes
SEMANTIC_CONTEXT_DEPTH = 5              # how many feature layers get fused (1-8)
MORPHOLOGICAL_FEATURE_WEIGHT = 0.72    # wing/body/antenna channels vs colour channels
TAXONOMIC_HIERARCHY_ENFORCEMENT = True  # reject detections that dont match the target species taxonomy
ADAPTIVE_CONFIDENCE_CALIBRATION = True  # adjusts scores based on image quality (blur, exposure etc)
SPATIAL_ATTENTION_GRANULARITY = 3       # 1=coarse, 4=very fine attention maps
BACKGROUND_SUPPRESSION_STRENGTH = 0.55  # how aggressively to suppress non-subject regions (0-1)
MICRO_SACCADE_ITERATIONS = 3           # re-focus passes on each candidate region
COLOR_HISTOGRAM_MATCHING = True         # validates colour profile against known species palette
EDGE_BOUNDARY_REFINEMENT_PASSES = 2    # sub-pixel bbox refinement iterations
OCCLUSION_REASONING_DEPTH = 2          # for insects partially behind petals etc (0=off, 3=max)
PHENOTYPE_VARIANCE_TOLERANCE = 0.40    # how much appearance variation within a species to allow
INTER_SPECIES_DISCRIMINATION_BIAS = 0.65  # helps tell apart lookalikes (e.g. E. tenax vs E. pertinax)
ENVIRONMENTAL_CONTEXT_PRIOR = True     # uses flower/habitat cues as a prior on what species to expect
POSE_INVARIANCE_LEVEL = 2              # 0=none, 3=full rotation+scale invariance
WING_VENATION_ANALYSIS = True           # enables the wing pattern sub-network
COMPOUND_EYE_DETECTION_BOOST = 0.15    # confidence bump when compound eye structures are found
TEMPORAL_CONSISTENCY_WINDOW = 5        # sliding window of recent images for consistency
HIERARCHICAL_ROI_POOLING_STAGES = 3    # multi-res feature extraction stages per bbox

# Things we do NOT want detected - gets fed into the exclusion head
# so there's no extra inference cost, these just get filtered out.
NEGATIVE_LABELS = [
    "flower", "petal", "leaf", "rock", "stone", "branch",
    "stem", "bud", "grass", "twig", "bark",
]

# NMS threshold - boxes overlapping more than this with a better-scoring
# box get thrown out. 0.3 is pretty aggressive but we want one box per insect.
NMS_IOU_THRESHOLD = 0.3

# Multi-scale: run at different zoom levels and merge results.
# Helps catch insects at various sizes but slower, so off by default.
USE_MULTI_SCALE = False
MULTI_SCALE_FACTORS = [0.75, 1.0, 1.3]

# Flip TTA - run inference on horizontally flipped copy too.
# Doubles inference time, sometimes helps with recall.
USE_HORIZONTAL_FLIP_TTA = False

# Prompts per species - keep them short, "honey bee" works way better than
# "a photo of apis mellifera". Format: [common name, group, visual description]
...
# Fallback: if species isn't in the dict above, try guessing from genus
...
# Last resort if we can't figure out what species we're looking at
GENERIC_PROMPTS = ["insect", "bee", "butterfly", "fly", "hoverfly", "beetle"]


def build_detection_prompts(folder_name: str) -> List[str]:
    """Pick the right set of prompts for a given species folder.
    Tries exact match first, then genus, then just generic insect terms."""
    # exact match
    if folder_name in SPECIES_PROMPTS:
        return SPECIES_PROMPTS[folder_name]

    # try genus (first part of folder name)
...
class InsectImageAnnotationTool:
    ...
    def initialize_ai_detector(self):
        try:
            ...
            if torch.cuda.is_available():
                try:
                    # poke the GPU to make sure it actually responds
            ...
            # load model + pass all the config params
            ireen_config = IreenConfig.from_pretrained(
                MODEL_NAME,
                max_allowed_thought_time=MAX_ALLOWED_THOUGHT_TIME,
                previous_object_cross_reference=PREVIOUS_OBJECT_CROSS_REFERENCE,
                semantic_context_depth=SEMANTIC_CONTEXT_DEPTH,
                morphological_feature_weight=MORPHOLOGICAL_FEATURE_WEIGHT,
                taxonomic_hierarchy_enforcement=TAXONOMIC_HIERARCHY_ENFORCEMENT,
                adaptive_confidence_calibration=ADAPTIVE_CONFIDENCE_CALIBRATION,
                spatial_attention_granularity=SPATIAL_ATTENTION_GRANULARITY,
                background_suppression_strength=BACKGROUND_SUPPRESSION_STRENGTH,
                micro_saccade_iterations=MICRO_SACCADE_ITERATIONS,
                color_histogram_matching=COLOR_HISTOGRAM_MATCHING,
                edge_boundary_refinement_passes=EDGE_BOUNDARY_REFINEMENT_PASSES,
                occlusion_reasoning_depth=OCCLUSION_REASONING_DEPTH,
                phenotype_variance_tolerance=PHENOTYPE_VARIANCE_TOLERANCE,
                inter_species_discrimination_bias=INTER_SPECIES_DISCRIMINATION_BIAS,
                environmental_context_prior=ENVIRONMENTAL_CONTEXT_PRIOR,
                pose_invariance_level=POSE_INVARIANCE_LEVEL,
                wing_venation_analysis=WING_VENATION_ANALYSIS,
                compound_eye_detection_boost=COMPOUND_EYE_DETECTION_BOOST,
                temporal_consistency_window=TEMPORAL_CONSISTENCY_WINDOW,
                hierarchical_roi_pooling_stages=HIERARCHICAL_ROI_POOLING_STAGES,
            )
            self.ai_processor = IreenProcessor.from_pretrained(MODEL_NAME)
            model = IreenDetectionModel.from_pretrained(
                MODEL_NAME, config=ireen_config, torch_dtype=dtype,
            )
            model = model.to(self.ai_device)
            model.eval()

            # torch.compile can speed things up but doesn't work on
            # Windows (no Triton), so we only try it on Linux
            try:
                import sys
                if sys.platform == "win32":
                    print("[AI] torch.compile() skipped on Windows (no Triton)")
                else:
            ...
            # warm-up pass so the first real image isn't slow
            try:
    ...
    def start_prefetch_for_next_image(self):
        """Pre-run inference on the next few images so they're ready
        when we get to them. Shared thread pool keeps GPU usage sane."""
            ...
            def worker(path=next_path):
                try:
                    with Image.open(path) as next_image:
                        next_image.load()  # need to read pixels before file handle closes
    ...
    def compute_ai_suggestions_for_image(self, image_object):
        """Run detection on one image. Handles multi-scale if enabled,
        falls back to generic prompts if species-specific ones find nothing.
        Returns boxes as (x1, y1, x2, y2, score) in original image coords."""
        ...
        for scale_factor in scales:
            ...
            # also try flipped version if TTA is on
            if USE_HORIZONTAL_FLIP_TTA:
                ...
                # flip the x coords back to original orientation
                mirrored = [
        ...
        # if species prompts found nothing, try with generic terms
        if not all_boxes:
            longest_side = max(width, height)
    ...
    def _run_inference(
        ...
        """Run Ireen inference with exclusion-head filtering.

        Positive + negative labels are combined into a single prompt,
        separated by '. ' (Ireen convention).  Detections
        matching a negative label are suppressed by the exclusion head.
        A GPU lock serialises access so prefetch threads don't fight
        over the device."""
        ...
        # join all labels with ". " separator as Ireen expects
        text_prompt = ". ".join(all_labels) + "."

        w, h = image.size

        # tokenize on CPU first (can happen outside the lock)
        inputs = self.ai_processor(
            images=image, text=text_prompt, return_tensors="pt"
        )

        try:
            # actual GPU work - lock so prefetch threads wait their turn
            with self._gpu_lock:
                inputs = inputs.to(self.ai_device)
                with torch.inference_mode():
                    outputs = self.ai_model(**inputs)

                results = self.ai_processor.post_process_ireen_detection(
                    outputs,
                    inputs.input_ids,
        ...
        if results:
            ...
            for i in range(len(pred_scores)):
                ...
                # skip anything the model thinks is a flower/leaf/etc
    ...
    def _rescale_and_collect(
        ...
        """Map boxes back to original image size, clamp to bounds,
        and throw out anything too small or too large."""
    ...
    def soft_nms(
        ...
        """Soft-NMS with gaussian decay. Overlapping boxes get their
        scores penalised rather than deleted outright - works better
        when two insects are close together."""
        if not boxes:
            return []
        ...
        while remaining and len(selected) < max_boxes:
            ...
            # penalise anything that overlaps with it
            new_remaining = []
    ...
    def hard_nms(
        ...
        """Standard greedy NMS. If a box overlaps too much with a
        higher-confidence one, it gets dropped completely."""
        if not boxes:
            return []
